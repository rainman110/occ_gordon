...
#include <internal/GordonSurfaceBuilder.h>
#include <internal/CurvesToSurface.h>
...
math_Matrix readMatrix(const std::string &filename)
{
    ...
    math_Matrix result(m_min, m_max, n_min, n_max);
...
TEST(BSplineAlgorithms, testCreateCommonKnotsVectorTolerance)
{
    ...
    curves = occ_gordon_internal::BSplineAlgorithms::createCommonKnotsVectorCurve(curves, 0.01);
...
TEST(BSplineAlgorithms, testCreateCommonKnotsVectorToleranceBug)
{
    ...
    curves = occ_gordon_internal::BSplineAlgorithms::createCommonKnotsVectorCurve(curves, 0.1);
    ASSERT_EQ(4, curves[0]->NbKnots());
    ASSERT_EQ(4, curves[1]->NbKnots());
...
TEST(BSplineAlgorithms, testReparametrizeBSplineContinuouslyApproxWithKink)
{
    ...
    newParms[1] = 0.6;
    Handle(Geom_BSplineCurve) splineRepar = occ_gordon_internal::BSplineAlgorithms::reparametrizeBSplineContinuouslyApprox(spline, oldParms, newParms, 5).curve;
...
TEST(BSplineAlgorithms, testCreateGordonSurfaceGeneral)
{
    ...
    Handle(Geom_BSplineSurface) gordonSurface = occ_gordon_internal::curveNetworkToSurface(splines_u_vector, splines_v_vector);
...
TEST(BSplineAlgorithms, findKinks)
{
    ...
    std::vector<double> kinks = occ_gordon_internal::BSplineAlgorithms::getKinkParameters(curve);
    ASSERT_EQ(1, kinks.size());
    EXPECT_NEAR(0.7, kinks[0], 1e-10);

    curve->InsertKnot(0.5, 3, 1e-10, true);
    kinks = occ_gordon_internal::BSplineAlgorithms::getKinkParameters(curve);
    ASSERT_EQ(1, kinks.size());
    EXPECT_NEAR(0.7, kinks[0], 1e-10);
}
...
TEST(BSplineAlgorithms, knotsFromParams)
{
    ...
    std::vector<double> knots = occ_gordon_internal::BSplineAlgorithms::knotsFromCurveParameters(params, degree, false);
    ...
    degree = 3;
    knots = occ_gordon_internal::BSplineAlgorithms::knotsFromCurveParameters(params, degree, false);
...
TEST(BSplineAlgorithms, knotsFromParamsClosed)
{
    ...
    unsigned int degree = 3;
    std::vector<double> knots = occ_gordon_internal::BSplineAlgorithms::knotsFromCurveParameters(params, degree, true);
    ...
    degree = 2;
    knots = occ_gordon_internal::BSplineAlgorithms::knotsFromCurveParameters(params, degree, true);
